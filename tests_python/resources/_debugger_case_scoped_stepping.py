...
user_global_ns = user_ns = user_module.__dict__
...
async def run_ast_nodes(
    ...
    for node, mode in to_run:
        ...
        asy = compare(code)
        if await run_code(code, async_=asy):
            return True


async def run_code(code_obj, *, async_=False):
    if async_:
        with tracing_info():
...
async def main():
    filename = os.path.join(os.path.dirname(__file__), '_debugger_case_scoped_stepping_target.py')
    ...
    await run_ast_nodes(code_ast.body, filename)
